...
public class TargetNineCounts extends TableImpl<TargetNineCountsRecord> {
     ...
     */
    public final TableField<TargetNineCountsRecord, String> TARGET_ID = createField(DSL.name("target_id"), SQLDataType.VARCHAR(36), this, "");

    /**
     * The column <code>public.target_nine_counts.count</code>.
     */
    public final TableField<TargetNineCountsRecord, Long> COUNT = createField(DSL.name("count"), SQLDataType.BIGINT, this, "");
    ...
    private TargetNineCounts(Name alias, Table<TargetNineCountsRecord> aliased, Field<?>[] parameters) {
        ...
        create view "target_nine_counts" as  SELECT t.id AS target_id,
          count(t.id) AS count
         FROM (source_nine s
           JOIN target_nine t ON (((t.id)::text = (s.target_id)::text)))
        GROUP BY t.id;
        """));
    }
    ...
    public <O extends Record> TargetNineCounts(Table<O> child, ForeignKey<O, TargetNineCountsRecord> key) {
        super(child, key, TARGET_NINE_COUNTS);
    }
    ...
    @Override
    public TargetNineCounts as(Table<?> alias) {
        return new TargetNineCounts(alias.getQualifiedName(), this);
    }
     ...
     */
    @Override
    public TargetNineCounts rename(Table<?> name) {
        return new TargetNineCounts(name.getQualifiedName(), null);
    }
